...
import { type VagaMapeada, type Programa } from "@/hooks/useOportunidades";
...
function ProgramaIcon({ programa }: { programa: Programa }) {
  const base =
    "w-14 h-14 rounded-xl flex items-center justify-center bg-[#dce8f7]";
  if (programa === "PID")
    return (
      <div className={base}>
        <GraduationCap size={25} className="text-[#00488C]" />
      </div>
    );
  if (programa === "PIBIC")
    return (
      <div className={base}>
        <FlaskConical size={25} className="text-[#00488C]" />
      </div>
    );
  if (programa === "P&D")
    return (
      <div className={base}>
        <MicroscopeIcon size={25} className="text-[#00488C]" />
      </div>
    );
  return (
    <div className={base}>
      <Monitor size={25} className="text-[#00488C]" />
    </div>
  );
}
  ...
  return (
    <div className="bg-[#F2F2F2] rounded-2xl px-6 py-5 font-sans">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <Button
            ...
            onClick={() => !comResultados && onSaberMais(vaga.id)}
            title={
              comResultados
                ? "Esta oportunidade já encerrou as inscrições"
                : undefined
            }
          >
            <Info size={14} />
            {comResultados ? "Encerrado" : "Saber mais"}
          </Button>
        </div>
      </div>

      {comResultados && (
        <div className="mt-3 flex items-center gap-2 text-xs text-gray-500 bg-gray-200 rounded-lg px-3 py-2">
          <FileText size={13} className="shrink-0" />
          <span>
            Resultados disponíveis —{" "}
            {vaga.resultados.map((r, i) => (
              <span key={r.id}>
                <a
                  href={r.link}
                  target="_blank"
                  rel="noreferrer"
                  className="underline hover:text-[#00488C] transition-colors"
                >
                  {r.titulo}
                </a>
                {i < vaga.resultados.length - 1 && ", "}
              </span>
            ))}
          </span>
        </div>
      ...
      <div className="mt-4 text-sm text-gray-700 leading-relaxed space-y-0.5">
        <p>{vaga.descricao}</p>
        <p>Coordenador responsável: {vaga.coordenador}.</p>
        <p>Valor: {vaga.remuneracao}</p>
      </div>
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <button
            ...
            className={cn(
              ...
              vaga.salvo
                ? "text-[#1a4fa0]"
                : "text-gray-400 hover:text-[#1a4fa0]",
            )}
            title={vaga.salvo ? "Remover dos salvos" : "Salvar vaga"}
          >
            <BookmarkIcon
              size={25}
              fill={vaga.salvo ? "currentColor" : "none"}
            />
          </button>
        </div>
      </div>
